...
'''
# predict
def make_dataset(raw_t1, raw_t2, label, test, target_files=None):
    imgs = []
    file_list = os.listdir(raw_t1)
    ...
    if target_files is not None:
        ...
        exist_files = set(file_list)
        missing_files = [f for f in target_files if f not in exist_files]

        if len(missing_files) > 0:
            raise FileNotFoundError(
                "以下指定图片在 {} 中不存在: {}".format(raw_t1, missing_files)
            )
...
class RsDataset(data.Dataset):
    def __init__(self,raw_t1,raw_t2,label,test=False,target_files=None,t1_transform=None,t2_transform=None,
            label_transform=None):
        imgs = make_dataset(raw_t1, raw_t2, label, test, target_files)
        self.imgs = imgs
        self.t1_transform = t1_transform
        self.t2_transform = t2_transform
        self.label_transform = label_transform
    ...
    def __getitem__(self, index):
        ...
        if self.label_transform is not None:
            img_y = self.label_transform(img_y)
        file_name = os.path.basename(t1_path)

        return img_t1, img_t2, img_y, file_name
